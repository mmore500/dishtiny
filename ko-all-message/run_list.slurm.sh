...
#SBATCH --job-name ko-all-message
...
OUTPUT_DIR="/mnt/scratch/mmore500/ko-all-message/seed=${SEED}"
CONFIG_DIR="/mnt/home/mmore500/dishtiny/ko-all-message/"
...
for POP_PATH in $POP_PATHS; do
  ...
  for f in xx*1 xx*3 xx*5 xx*7 xx*9; do
    sed -i -- "s/\"id\": 96\$\|\"id\": 97\$\|\"id\": 119\$\|\"id\": 120\$/\"id\": 27/g" $f &
  ...
  for f in xx*0 xx*2 xx*4 xx*6 xx*8; do
    sed -i -- "s/\"id\": 89\$\|\"id\": 90\$\|\"id\": 91\$\|\"id\": 92\$\|\"id\": 93\$/\"id\": 27/g" $f &
    while [ $(jobs -r | wc -l) -gt 100 ]; do sleep 1; done
  done
