#pragma once
#ifndef DISH2_INTROSPECTION_COUNT_UNIQUE_MODULE_REGULATION_PROFILES_HPP_INCLUDE
#define DISH2_INTROSPECTION_COUNT_UNIQUE_MODULE_REGULATION_PROFILES_HPP_INCLUDE
...
size_t count_unique_module_regulation_profiles(
  const dish2::ThreadWorld< Spec >& world
) {

  const auto& population = world.population;

  using profile_t = emp::vector<float>;
  ...
  std::for_each(
    ...
    [&]( const auto& cell ){ for (const auto& cardinal : cell) {
      ...
      res.insert( sgpl::summarize_module_regulation(
        cardinal.cpu,
        cell.genome->program
      ) );
...
#endif // #ifndef DISH2_INTROSPECTION_COUNT_UNIQUE_MODULE_REGULATION_PROFILES_HPP_INCLUDE
