#pragma once
#ifndef DISH2_INTROSPECTION_GET_HIGHEST_ROOT_PREVALENT_CODING_GENOTYPE_HPP_INCLUDE
#define DISH2_INTROSPECTION_GET_HIGHEST_ROOT_PREVALENT_CODING_GENOTYPE_HPP_INCLUDE
...
#include "get_highest_root_id.hpp"
...
get_highest_root_prevalent_coding_genotype(
  const dish2::ThreadWorld<Spec>& world
) {

  const size_t highest_root_id = dish2::get_highest_root_id<Spec>( world );
  ...
  uitsl::for_each(
    ...
    [&counter, highest_root_id](
      const auto& coding_genotype, const size_t root_id
    ){
      if (root_id == highest_root_id) ++counter[ coding_genotype ];
    }
  );
...
#endif // #ifndef DISH2_INTROSPECTION_GET_HIGHEST_ROOT_PREVALENT_CODING_GENOTYPE_HPP_INCLUDE
