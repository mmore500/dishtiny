#pragma once
#ifndef DISH2_VIZ_FILL_COLORMAPS_PCABINARYEXPRESSIONCOLORMAP_HPP_INCLUDE
#define DISH2_VIZ_FILL_COLORMAPS_PCABINARYEXPRESSIONCOLORMAP_HPP_INCLUDE
...
class PcaBinaryExpressionColorMap {
...
public:
  ...
  void Refresh() {
    ...
    std::transform(
      DATA( raw_expression_summary ),
      DATA( raw_expression_summary )
        + raw_expression_summary->n_row * raw_expression_summary->n_col
      ,
      DATA( raw_expression_summary ),
    ...
    hopca::PCA module_expression_pca{ std::min(
      3ul, uitsl::audit_cast<size_t>( binary_expression_summary.value()->n_row )
    ) };

    const auto condensed_summary = hopca::drop_homogenous_columns(
      module_expression_pca.doPCA( *binary_expression_summary )
    );
...
#endif // #ifndef DISH2_VIZ_FILL_COLORMAPS_PCABINARYEXPRESSIONCOLORMAP_HPP_INCLUDE
