...
python3 "${script_dir}/../script/collate.py" "s3://${bucket}/endeavor=${endeavor}/perturbation-${state_target}-state-${perturbation}-competitions/stage=1+what=generated/stint=${stint}/" 'a=coalescence_result\+proc=[0-9]+\+replicate=[0-9]+\+.*\+stint=[0-9]+\+thread=[0-9]+\+ext=\.csv' \
