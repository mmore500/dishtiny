...
def RenderAndSave(upd, filename):
    ...
    for idx in range(own.size):
        for dir, drct in dirs.items():
            type = NONE
            if ppos[idx] == drct[idx] and cage[idx] < cage[drct[idx]] and chans[-1][drct[idx]] != chans[-1][idx]:
                type = CHILD
            elif ppos[drct[idx]] == idx and cage[drct[idx]] < cage[idx] and chans[-1][drct[idx]] != chans[-1][idx]:
                type = PARENT
            else:
