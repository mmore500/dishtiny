...
EMP_BUILD_CONFIG(
  ...
  VALUE(PROGRAM_MAX_FUN_CNT, size_t, 12, "Used for generating SGP programs. How many functions do we generate?"),
  ...
  VALUE(ENV_TRIG_FREQ, size_t, 4, "Fire environmental triggers every X updates."),

  GROUP(MUTATION_GROUP, "SignalGP Mutation Settings"),
  VALUE(MUTATION_RATE, double, 0.01, "What percentage of reproductions should be subject to mutations at all?"),
  VALUE(PROPAGULE_MUTATION_RATE, double, 0.5, "What percentage of propagule reproductions should be subject to mutations at all?"),
  ...
  VALUE(RUN_LENGTH, size_t, 1200100, "How many updates should we run the experiment for?"),
