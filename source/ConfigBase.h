...
EMP_BUILD_CONFIG(
  ...
  VALUE(PROGRAM_MIN_FUN_CNT, size_t, 1, "Used for generating SGP programs. At least, how many functions do we generate?"),
  ...
  VALUE(ENV_TRIG_FREQ, size_t, 4, "How often to fire environmental trigger events?"),
  ...
  VALUE(PROPAGULE_MUTATION_RATE, double, 0.5, "What percentage of propagule offspring should experience mutations?"),
  ...
  VALUE(SNAPSHOT_LENGTH, size_t, 100, "[NATIVE] How long should snapshots last for?"),
  VALUE(RUN_LENGTH, size_t, 1200100, "[NATIVE] How many updates should we run the experiment for?"),
