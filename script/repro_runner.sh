...
if [ -n "${repo_sha}" ]; then
  ...
  curl --fail --show-error --silent --location --retry 10 --retry-delay 1 --max-time 15 --connect-timeout 20 "https://raw.githubusercontent.com/mmore500/dishtiny/${repo_sha}/script/gitget.sh" -o "${gitget_path}"
  echo "wget success"
  chmod +x "${gitget_path}"
