...
variant_competitions, = bucket_handle.objects.filter(
    Prefix=f'endeavor={endeavor}/variant-competitions-focalbb/stage=4+what=collated/stint={stint}/'
)
...
for series, critical_sites in critical_sites_by_series.items():
    ...
    out_path = f'endeavor={endeavor}/genomesbb/stage=5+what=tabulated/stint={stint}/series={series}/a=genome+criteria=abundance+morph=noncritical_nopoutbb+proc=0+series={series}+stint={stint}+thread=0+variation=master+ext=.json.gz'
