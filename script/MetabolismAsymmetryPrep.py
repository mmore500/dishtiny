...
def GroupConflict(filename):
    ...
    res = defaultdict( # this channel group
        lambda: defaultdict( # other channel group
            lambda: defaultdict( # update
                lambda: (
                    [], # metabolism observations (repr or no)
                    set() # potential parent cells
                )
            )
        )
    )
            ...
            for neigh in neighs:
                if (
                    ...
                    and not ( # not cell parent
                        ...
                        and cages[i] < cages[indices[neigh[i]]]
                        ...
                        res[
                            ...
                            ppos[i] if (cages[i] < 8) else i
                        )

    proc = {
        ch_a : {
            ch_b : [
                # meatabolism rate per border cell
                sum(upd[0]) / len(upd[1])
                for upd in upds.values()
                if len(upd[0])
            ]

            for ch_b, upds in dict_b.items()
        }
        for ch_a, dict_b in res.items()
    }
