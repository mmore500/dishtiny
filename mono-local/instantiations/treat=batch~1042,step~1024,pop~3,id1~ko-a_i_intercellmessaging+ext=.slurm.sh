...
#SBATCH --job-name batch~1042,step~1024,pop~3,id1~ko-a_i_intercellmessaging
...
OUTPUT_DIR="/mnt/scratch/mmore500/mono-local/treat=batch~1042,step~1024,pop~3,id1~ko-a_i_intercellmessaging+seed=${SEED}"
...
mkdir -p ${OUTPUT_DIR}
cd ${OUTPUT_DIR}
tar -xvf "${CONFIG_DIR}/treat=batch~1042,step~1024,pop~3,id1~ko-a_i_intercellmessaging+ext=.tar.gz"
...
./dishtiny -SEED $SEED -SEED_POP 1 -RUN_LENGTH 0 -SNAPSHOT_LENGTH 8192        \
