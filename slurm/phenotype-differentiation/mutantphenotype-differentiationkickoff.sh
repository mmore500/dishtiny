...
second_subject_attrs: "mutate_on_load=1+"
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/mutant-phenotype-differentiation/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: "\${REPLICATE}"
endeavor: "\${ENDEAVOR}"
stint: "${STINT}"
...
dishtiny/script/slurm_stoker_containerized_kickoff.sh "${BUCKET}" "${CONTAINER_TAG}" "${REPO_SHA}" "mutant-phenotype-differentiation~configpack%${CONFIGPACK}~series%${SERIES%% *}...~stint%${STINT}"
