...
################################################################################
echo
echo "running restintcontrol-mutate_on_load.sh"
echo "------------------------"
################################################################################
...
if (( "$#" < 6 )); then
  echo "USAGE: [read_bucket] [write_bucket] [configpack] [container_tag] [repo_sha]  [mutate_on_load_root0] [mutate_on_load_root1] [stint] [series...]"
  exit 1
fi
...
MUTATE_ON_LOAD_ROOT0="${1}"
echo "MUTATE_ON_LOAD_ROOT0 ${MUTATE_ON_LOAD_ROOT0}"
shift

MUTATE_ON_LOAD_ROOT1="${1}"
echo "MUTATE_ON_LOAD_ROOT1 ${MUTATE_ON_LOAD_ROOT1}"
shift
...
wget --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 -t 10 -qO- "https://raw.githubusercontent.com/mmore500/dishtiny/${REPO_SHA}/slurm/restint/restintcontroljob.slurm.sh.jinja" | head
wget --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 -t 10 -qO- "https://raw.githubusercontent.com/mmore500/dishtiny/${REPO_SHA}/slurm/restint/restintcontroljob.slurm.sh.jinja" > "\${TEMPLATE_PATH}"
...
repo_sha: "${REPO_SHA}"
mutate_on_load_root0: "${MUTATE_ON_LOAD_ROOT0}"
mutate_on_load_root1: "${MUTATE_ON_LOAD_ROOT1}"
