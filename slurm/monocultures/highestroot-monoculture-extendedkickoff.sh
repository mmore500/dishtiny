...
################################################################################
echo
echo "running highestroot-monoculturekickoff.sh"
echo "------------------------"
################################################################################
...
for STINT in ${STINTS}; do
  ...
  j2 --format=yaml -o "\${JOB_SCRIPT}" "dishtiny/slurm/monocultures/highestroot-monoculture-extendedjob.slurm.sh.jinja" << J2_HEREDOC_EOF
bucket: "${BUCKET}"
