...
if (( "$#" < 8 )); then
  echo "USAGE: [bucket_read] [bucket_write] [configpack] [container_tag] [repo_sha] [stint] [stint_stop] [series...]"
  exit 1
fi

BUCKET_READ="${1}"
echo "BUCKET_READ ${BUCKET_READ}"
shift

BUCKET_WRITE="${1}"
echo "BUCKET_WRITE ${BUCKET_READ}"
shift
...
echo "BUCKET_READ ${BUCKET_READ}"
echo "BUCKET_WRITE ${BUCKET_WRITE}"
echo "CONFIGPACK ${CONFIGPACK}"
echo "CONTAINER_TAG ${CONTAINER_TAG}"
echo "REPO_SHA ${REPO_SHA}"
echo "STINT ${STINT}"
...
for just_one_series in ${SERIES}; do
  ...
  j2 --format=yaml -o "\${JOB_SCRIPT}" "dishtiny/slurm/replay/replayjob.slurm.sh.jinja" << J2_HEREDOC_EOF
