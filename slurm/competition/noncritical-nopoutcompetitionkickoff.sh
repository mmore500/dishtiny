...
################################################################################
echo
echo "running noncritical-nopoutcompetitionkickoff.sh"
echo "-----------------------------------------------------"
################################################################################
...
for JUST_ONE_SERIES in ${SERIES}; do
  for REPLICATE in 0; do
    ...
    SECOND_COMPETITOR="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=3+what=tabulated/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=genome+criteria=abundance+morph=noncritical_nopout+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.json.gz"
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/noncritical-nopout-competitions/stage=4+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: "\${REPLICATE}"
endeavor: "\${ENDEAVOR}"
stint: "${STINT}"
