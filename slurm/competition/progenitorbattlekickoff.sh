...
################################################################################
echo
echo "running progenitorbattlekickoff.sh"
echo "-----------------------------------"
################################################################################
...
for JUST_ONE_SERIES in ${SERIES}; do
  for REPLICATE in {0..9}; do
    ...
    FIRST_COMPETITOR="s3://dnh2v/endeavor=\${ENDEAVOR}/evolve/populations/stage=0+what=generated/stint=0/series=\${JUST_ONE_SERIES}/a=population+proc=0+series=\${JUST_ONE_SERIES}+stint=0+thread=0+variation=master+ext=.bin.xz"
    SECOND_COMPETITOR="s3://dnh2v/endeavor=\${ENDEAVOR}/evolve/populations/stage=0+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=population+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.bin.xz"
...
output_url: "s3://dnh2v/endeavor=\${ENDEAVOR}/progenitor-battles/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: "\${REPLICATE}"
endeavor: "\${ENDEAVOR}"
stint: "${STINT}"
J2_HEREDOC_EOF
