...
################################################################################
echo
echo "running controlcompetition-dosecorrected-nodiversitymaintenance-withfuturebioticbackground-withmutkickofff.sh"
echo "-------------------------------------------------------------------------"
################################################################################
...
for JUST_ONE_SERIES in ${SERIES}; do
  for REPLICATE in {0..19}; do
    ...
    j2 --format=yaml -o "a=competition+series=\${JUST_ONE_SERIES}+stint=${STINT}+replicate=\${REPLICATE}+ext=.slurm.sh" "dishtiny/slurm/competition/bioticbackgroundcompetitionjob.slurm.sh.jinja" << J2_HEREDOC_EOF
bucket: ${BUCKET}
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/control-competitions-dosecorrected-nodiversitymaintenance-withfuturebioticbackground-withmut/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: "\${REPLICATE}"
endeavor: "\${ENDEAVOR}"
