...
################################################################################
echo
echo "running bioticbackground-fitnesscomplexity-fromnobbphenequiv-competitionkickoff.sh"
echo "-------------------------------------------------------------------------"
################################################################################
...
echo
echo "STINT \${STINT}:"
echo "Download phenotype_equivalent_nopout genome"
echo "----------------------------------------------------------"
################################################################################

PE_NOPOUT_URI="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=generated/stint=\${STINT}/series=${SERIES}/a=genome+criteria=abundance+morph=jenga_phenotype_equivalent_nopout+proc=0+series=${SERIES}+stint=\${STINT}+thread=0+variation=master+ext=.json.gz"
PHENOTYPE_EQUIVALENT_GENOME="\$(mktemp -d)/\$(basename \${PE_NOPOUT_URI})"

echo "downloading \${PE_NOPOUT_URI} to \${PHENOTYPE_EQUIVALENT_GENOME}"
for retry in {1..20}; do
  aws s3 cp --quiet \
    "\${PE_NOPOUT_URI}" \
    "\${PHENOTYPE_EQUIVALENT_GENOME}" \
    && echo "  \${PE_NOPOUT_URI} download success" && break \
    || (echo "retrying \${PE_NOPOUT_URI} download (\${retry})" && sleep \$((RANDOM % 10)))
  if ((\${retry}==20)); then echo "\${PE_NOPOUT_URI} download fail" && exit 123123; fi
done
...
for p in *a=genome*morph=wildtype*variation=* ; do
  ...
  for retry in {1..20}; do
    aws s3 cp --quiet \
      "\${p}" \
      "s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes-bb-fromnobbphenequiv/stage=2+what=generated/stint=\${STINT}/series=${SERIES}/\${f}" \
...
second_competitor_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes-bb-fromnobbphenequiv/stage=2+what=generated/stint=\${STINT}/series=${SERIES}/\${variant}"
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/variant-competitions-backgroundbb-fromnobbphenequiv/stage=3+what=generated/stint=\${STINT}/series=${SERIES}/"
replicate: 0
endeavor: \${ENDEAVOR}
stint: \${STINT}
series: ${SERIES}
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/variant-competitions-focalbb-fromnobbphenequiv/stage=3+what=generated/stint=\${STINT}/series=${SERIES}/"
replicate: 0
endeavor: \${ENDEAVOR}
stint: \${STINT}
series: ${SERIES}
