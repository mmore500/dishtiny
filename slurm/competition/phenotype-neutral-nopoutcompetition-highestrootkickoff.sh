...
################################################################################
echo
echo "running phenotype-neutral-nopoutcompetition-highestrootkickoff.sh"
echo "-----------------------------------------------------"
################################################################################
...
for JUST_ONE_SERIES in ${SERIES}; do
  for REPLICATE in 0; do
    ...
    FIRST_COMPETITOR="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=extracted/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=genome+criteria=abundance_highest_root+morph=wildtype+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.json.gz"
    SECOND_COMPETITOR="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=2+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=genome+criteria=abundance_highest_root+morph=phenotype_equivalent_nopout+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.json.gz"
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/phenotype-neutral-nopout-competitions-highestroot/stage=3+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: "\${REPLICATE}"
endeavor: "\${ENDEAVOR}"
stint: "${STINT}"
