...
################################################################################
echo
echo "running progenitorcompetitionkickoff.sh"
echo "-----------------------------------"
################################################################################
...
for JUST_ONE_SERIES in ${SERIES}; do
  for REPLICATE in {0..9}; do
    ...
    FIRST_COMPETITOR="s3://dnh2v/endeavor=\${ENDEAVOR}/genomes/stage=0+what=generated/stint=0/series=\${JUST_ONE_SERIES}/a=genome+criteria=abundance+morph=wildtype+proc=0+series=\${JUST_ONE_SERIES}+stint=0+thread=0+variation=master+ext=.json.gz"
...
output_url: "s3://dnh2v/endeavor=\${ENDEAVOR}/progenitor-competitions/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: "\${REPLICATE}"
endeavor: "\${ENDEAVOR}"
stint: "${STINT}"
J2_HEREDOC_EOF
