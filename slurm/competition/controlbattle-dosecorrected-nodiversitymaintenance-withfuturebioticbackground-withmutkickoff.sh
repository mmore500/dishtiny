...
################################################################################
echo
echo "running controlbattle-dosecorrected-nodiversitymaintenance-withfuturebioticbackground-withmutkickofff.sh"
echo "-------------------------------------------------------------------------"
################################################################################
...
for JUST_ONE_SERIES in ${SERIES}; do
  for REPLICATE in {0..19}; do
    ...
    FIRST_COMPETITOR="s3://${BUCKET}/endeavor=\${ENDEAVOR}/evolve/populations/stage=0+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=population+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.bin.xz"
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/control-battles-dosecorrected-nodiversitymaintenance-withfuturebioticbackground-withmut/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: "\${REPLICATE}"
endeavor: "\${ENDEAVOR}"
