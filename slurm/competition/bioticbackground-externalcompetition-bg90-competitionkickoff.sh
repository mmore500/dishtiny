...
################################################################################
echo
echo "running bioticbackground-externalcompetition-bg90-competitionkickoff.sh"
echo "-------------------------------------------------------------------------"
################################################################################
...
JOB_SCRIPT="\$( keyname pack \
  --a "externalcompetition_bg90_focalbb" --comparator_series "\${comparator_series}" --replicate "\${replicate}" --counter "\${counter}" --ext ".slurm.sh" \
)"
...
first_background_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/evolve/populations/stage=0+what=generated/stint=90/series=${SERIES}/a=population+proc=0+series=${SERIES}+stint=90+thread=0+variation=master+ext=.bin.xz"
first_background_attrs: "dose=2+filter_lowest_root+"
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/external-competitions-bg90-focalbb/stage=1+what=generated/stint=\${STINT}/series=${SERIES}/"
replicate: \${replicate}
endeavor: \${ENDEAVOR}
stint: \${STINT}
series: ${SERIES}
