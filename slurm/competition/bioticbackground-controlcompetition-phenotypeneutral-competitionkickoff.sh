...
################################################################################
echo
echo "running bioticbackground-controlcompetition-phenotypeneutral-competitionkickoff.sh"
echo "-------------------------------------------------------------------------"
################################################################################
...
echo
echo "STINT \${STINT}:"
echo "Generate PhenNeut vs PhenNeut (control) Focal BB Competition Runscripts"
echo "-------------------------------------------------------------------------"
################################################################################
...
JOB_SCRIPT="\$( keyname pack \
  --a "phenneut_vs_phenneut_focalbb" --replicate "\${replicate}" --ext ".slurm.sh" \
)"
...
repo_sha: ${REPO_SHA}
first_competitor_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=generated/stint=\${STINT}/series=${SERIES}/a=genome+bgpop=1+criteria=abundance+morph=jenga_phenotype_equivalent_nopout_with_biotic_background+proc=0+series=${SERIES}+stint=\${STINT}+thread=0+variation=master+ext=.json.gz"
second_competitor_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=generated/stint=\${STINT}/series=${SERIES}/a=genome+bgpop=1+criteria=abundance+morph=jenga_phenotype_equivalent_nopout_with_biotic_background+proc=0+series=${SERIES}+stint=\${STINT}+thread=0+variation=master+ext=.json.gz"
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/control-competitions-focalbb-phenotypeneutral/stage=2+what=generated/stint=\${STINT}/series=${SERIES}/"
replicate: \${replicate}
endeavor: \${ENDEAVOR}
stint: \${STINT}
series: ${SERIES}
