...
################################################################################
echo
echo "running bioticbackground-highestroot-fitnesscomplexity-competitionkickoff.sh"
echo "-------------------------------------------------------------------------"
################################################################################
...
WILDTYPE_URI="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=extracted/stint=\${STINT}/series=${SERIES}/a=genome+criteria=abundance_highest_root+morph=wildtype+proc=0+series=${SERIES}+stint=\${STINT}+thread=0+variation=master+ext=.json.gz"
...
BB_PE_NOPOUT_URI="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=generated/stint=\${STINT}/series=${SERIES}/a=genome+bgpop=1+criteria=abundance_highest_root+morph=jenga_phenotype_equivalent_nopout_with_biotic_background+proc=0+series=${SERIES}+stint=\${STINT}+thread=0+variation=master+ext=.json.gz"
...
for p in *a=genome*morph=wildtype*variation=* ; do
  ...
  for retry in {1..20}; do
    aws s3 cp --quiet \
      "\${p}" \
      "s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes-highestroot-bb/stage=2+what=generated/stint=\${STINT}/series=${SERIES}/\${f}" \
...
echo
echo "STINT \${STINT}:"
echo "Generate WT vs Variant Background BB Competition Runscripts"
echo "---------------------------------------------"
################################################################################
...
repo_sha: ${REPO_SHA}
first_competitor_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=extracted/stint=\${STINT}/series=${SERIES}/\${wt}"
second_competitor_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes-highestroot-bb/stage=2+what=generated/stint=\${STINT}/series=${SERIES}/\${variant}"
...
first_background_attrs: "dose=2+filter_lowest_root+"
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/variant-competitions-highestroot-backgroundbb/stage=3+what=generated/stint=\${STINT}/series=${SERIES}/"
replicate: 0
endeavor: \${ENDEAVOR}
stint: \${STINT}
series: ${SERIES}
...
echo
echo "STINT \${STINT}:"
echo "Generate WT vs Variant Focal BB Competition Runscripts"
echo "-------------------------------------------------------------------------"
################################################################################
...
JOB_SCRIPT="\$( keyname pack \
  --a "wt_vs_variant_focalbb" --variation "\${variation}" --ext ".slurm.sh" \
)"
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/variant-competitions-highestroot-focalbb/stage=3+what=generated/stint=\${STINT}/series=${SERIES}/"
replicate: 0
endeavor: \${ENDEAVOR}
stint: \${STINT}
series: ${SERIES}
...
echo
echo "STINT \${STINT}:"
echo "Generate WT vs WT (control) Background BB Competition Runscripts"
echo "---------------------------------------------"
################################################################################
...
repo_sha: ${REPO_SHA}
first_competitor_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=extracted/stint=\${STINT}/series=${SERIES}/\${wt}"
second_competitor_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=extracted/stint=\${STINT}/series=${SERIES}/\${wt}"
first_background_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/evolve/populations/stage=0+what=generated/stint=\${STINT}/series=${SERIES}/a=population+proc=0+series=${SERIES}+stint=\${STINT}+thread=0+variation=master+ext=.bin.xz"
first_background_attrs: "dose=2+filter_lowest_root+"
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/control-competitions-highestroot-backgroundbb/stage=3+what=generated/stint=\${STINT}/series=${SERIES}/"
replicate: \${replicate}
endeavor: \${ENDEAVOR}
stint: \${STINT}
series: ${SERIES}
...
echo
echo "STINT \${STINT}:"
echo "Generate WT vs WT (control) Focal BB Competition Runscripts"
echo "-------------------------------------------------------------------------"
################################################################################
...
JOB_SCRIPT="\$( keyname pack \
  --a "wt_vs_wt_focalbb" --replicate "\${replicate}" --ext ".slurm.sh" \
)"
...
first_background_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/evolve/populations/stage=0+what=generated/stint=\${STINT}/series=${SERIES}/a=population+proc=0+series=${SERIES}+stint=\${STINT}+thread=0+variation=master+ext=.bin.xz"
first_background_attrs: "dose=2+filter_except_lowest_root+"
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/control-competitions-highestroot-focalbb/stage=3+what=generated/stint=\${STINT}/series=${SERIES}/"
replicate: \${replicate}
endeavor: \${ENDEAVOR}
stint: \${STINT}
series: ${SERIES}
