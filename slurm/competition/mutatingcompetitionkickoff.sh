...
################################################################################
echo
echo "running mutatingcompetitionkickoff.sh"
echo "-----------------------------------"
################################################################################
...
first_competitor_attrs: "set_mutation_occurence_rate_multiplicand=0+"
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/mutating-competitions/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: "\${REPLICATE}"
endeavor: "\${ENDEAVOR}"
stint: "${STINT}"
...
dishtiny/script/slurm_stoker_containerized_kickoff.sh "${BUCKET}" "${CONTAINER_TAG}" "${REPO_SHA}" "mutating-competition~configpack%${CONFIGPACK}~series%${SERIES%% *}...~stint%${STINT}"
