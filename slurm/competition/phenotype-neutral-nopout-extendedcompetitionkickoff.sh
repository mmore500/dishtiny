...
################################################################################
echo
echo "running phenotype-neutral-nopout-extendedcompetitionkickoff.sh"
echo "-----------------------------------------------------"
################################################################################
...
for JUST_ONE_SERIES in ${SERIES}; do
  for REPLICATE in 0; do
    ...
    SECOND_COMPETITOR="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes-extended/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=genome+criteria=abundance+morph=phenotype_equivalent_nopout+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.json.gz"
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/phenotype-neutral-nopout-extended-competitions/stage=2+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: "\${REPLICATE}"
run_seconds: 3600
endeavor: "\${ENDEAVOR}"
stint: "${STINT}"
