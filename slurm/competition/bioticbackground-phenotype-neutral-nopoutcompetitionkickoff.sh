...
################################################################################
echo
echo "running bioticbackground-phenotype-neutral-nopoutcompetitionkickoff.sh"
echo "----------------------------------------------------------------------"
################################################################################
...
for JUST_ONE_SERIES in ${SERIES}; do
  for REPLICATE in 0; do
    ...
    SECOND_COMPETITOR="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=genome+bgpop=1+criteria=abundance+morph=jenga_phenotype_equivalent_nopout_with_biotic_background+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.json.gz"
...
first_background_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/evolve/populations/stage=0+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=population+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.bin.xz"
first_background_attrs: "dose=2+filter_except_lowest_root+"
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/bioticbackground-phenotype-neutral-nopout-competitions/stage=2+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: \${REPLICATE}
endeavor: \${ENDEAVOR}
stint: ${STINT}
