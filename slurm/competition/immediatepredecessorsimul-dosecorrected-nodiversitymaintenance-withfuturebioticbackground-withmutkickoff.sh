...
################################################################################
echo
echo "running immediatepredecessorsimul-dosecorrected-nodiversitymaintenance-withfuturebioticbackground-withmutkickofff.sh"
echo "-------------------------------------------------------------------------"
################################################################################
...
################################################################################
echo
echo "running predecessorsimulkickoff.sh"
echo "----------------------------------------"
################################################################################
...
PREDECESSOR_STINT="$(( STINT > 1 ? STINT - 1 : 0 ))"
...
for JUST_ONE_SERIES in ${SERIES}; do
  for REPLICATE in {0..19}; do
    ...
    FIRST_BACKGROUND="s3://${BUCKET}/endeavor=\${ENDEAVOR}/evolve/populations/stage=0+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=population+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.bin.xz"
    ...
    j2 --format=yaml -o "a=simul+series=\${JUST_ONE_SERIES}+stint=${STINT}+replicate=\${REPLICATE}+ext=.slurm.sh" "dishtiny/slurm/competition/bioticbackgroundcompetitionjob.slurm.sh.jinja" << J2_HEREDOC_EOF
bucket: ${BUCKET}
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/immediatepredecessor-simuls-dosecorrected-nodiversitymaintenance-withfuturebioticbackground-withmut/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: "\${REPLICATE}"
endeavor: "\${ENDEAVOR}"
series: "\${JUST_ONE_SERIES}"
stint: "${STINT}"
enable_mutation: true
disable_difersity_maintenance: true
J2_HEREDOC_EOF
