...
for JUST_ONE_SERIES in ${SERIES}; do
  for REPLICATE in {0..9}; do
    ...
    FIRST_COMPETITOR="s3://dnh2v/endeavor=\${ENDEAVOR}/genomes/stage=0+what=generated/stint=\${PREDECESSOR_STINT}/series=\${JUST_ONE_SERIES}/a=genome+criteria=abundance+morph=wildtype+proc=0+series=\${JUST_ONE_SERIES}+stint=\${PREDECESSOR_STINT}+thread=0+variation=master+ext=.json.gz"
