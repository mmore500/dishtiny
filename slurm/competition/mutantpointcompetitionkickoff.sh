...
second_competitor_attrs: "point_mutate_on_load=1+"
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/mutant-point-competitions/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/"
replicate: "\${REPLICATE}"
endeavor: "\${ENDEAVOR}"
stint: "${STINT}"
...
dishtiny/script/slurm_stoker_containerized_kickoff.sh "${BUCKET}" "${CONTAINER_TAG}" "${REPO_SHA}" "mutant-point-competition~configpack%${CONFIGPACK}~series%${SERIES%% *}...~stint%${STINT}"
