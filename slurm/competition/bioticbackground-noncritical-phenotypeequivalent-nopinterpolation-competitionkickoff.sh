...
################################################################################
echo
echo "running bioticbackground-noncritical-phenotypeequivalent-nopinterpolation-competitionkickoff.sh"
echo "-----------------------------------"
################################################################################
...
for JUST_ONE_SERIES in ${SERIES}; do
  ...
  python3 \${REPRO_WORK_DIRECTORY}/dishtiny/script/make_nop_interpolated_variants.py "\${LESS_OPS_PATH}" "\${MORE_OPS_PATH}" 90 30
...
for cur_genome in *a=genome*+ext=.json.gz; do
  ...
  j2 --format=yaml -o "a=competition+nop_interpolation_step=\${cur_interpolation_step}+series=\${cur_series}+stint=${STINT}+ext=.slurm.sh" "\${REPRO_WORK_DIRECTORY}/dishtiny/slurm/competition/bioticbackgroundcompetitionjob.slurm.sh.jinja" << J2_HEREDOC_EOF
bucket: ${BUCKET}
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/bioticbackground-noncritical-phenotypeneutral-nopinterpolation-competitions/stage=7+what=generated/stint=${STINT}/series=\${cur_series}/"
replicate: 0
endeavor: \${ENDEAVOR}
stint: ${STINT}
series: \${cur_series}
...
\${REPRO_WORK_DIRECTORY}/dishtiny/script/slurm_stoker_containerized_kickoff.sh "${BUCKET}" "${CONTAINER_TAG}" "${REPO_SHA}" "bb-noncritical-phenotypeequivalent-nopinterpolation-competition~configpack%${CONFIGPACK}~series%${SERIES%% *}...~stint%${STINT}"
