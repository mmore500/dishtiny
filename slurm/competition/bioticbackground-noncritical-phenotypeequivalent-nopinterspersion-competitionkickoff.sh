...
################################################################################
echo
echo "running bioticbackground-noncritical-phenotypeequivalent-nopinterspersion-competitionkickoff.sh"
echo "-----------------------------------"
################################################################################
...
################################################################################
echo
echo "running noncritical-phenotypeequivalent-nopinterspersion-competitionkickoff.sh"
echo "-----------------------------------"
################################################################################
...
for JUST_ONE_SERIES in ${SERIES}; do
  ...
  MORE_OPS_URL="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=genome+bgpop=1+criteria=abundance+morph=jenga_phenotype_equivalent_nopout_with_biotic_background+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.json.gz"
  LESS_OPS_URL="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomesbb/stage=5+what=tabulated/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=genome+criteria=abundance+morph=noncritical_nopoutbb+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.json.gz"
...
for p in *a=genome*+ext=.json.gz; do
  ...
  for retry in {1..20}; do
    aws s3 cp --quiet \
      "\${p}" \
      "s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomesbb/stage=6+what=tabulated/stint=${STINT}/series=\${cur_series}/" \
...
################################################################################
echo
echo "Generate interspersion Competition Runscripts"
echo "---------------------------------------------"
################################################################################
...
for cur_genome in *a=genome*+ext=.json.gz; do
  ...
  cur_interspersion_step="\$(echo \${cur_genome} | keyname extract nop_interspersion_step)"

  FIRST_COMPETITOR="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=1+what=generated/stint=${STINT}/series=\${JUST_ONE_SERIES}/a=genome+bgpop=1+criteria=abundance+morph=jenga_phenotype_equivalent_nopout_with_biotic_background+proc=0+series=\${JUST_ONE_SERIES}+stint=${STINT}+thread=0+variation=master+ext=.json.gz"
  SECOND_COMPETITOR="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomesbb/stage=6+what=tabulated/stint=${STINT}/series=\${cur_series}/\${cur_genome}"

  echo "FIRST_COMPETITOR \${FIRST_COMPETITOR}"
  echo "SECOND_COMPETITOR \${SECOND_COMPETITOR}"

  j2 --format=yaml -o "a=competition+nop_interspersion_step=\${cur_interspersion_step}+series=\${cur_series}+stint=${STINT}+ext=.slurm.sh" "\${REPRO_WORK_DIRECTORY}/dishtiny/slurm/competition/bioticbackgroundcompetitionjob.slurm.sh.jinja" << J2_HEREDOC_EOF
bucket: ${BUCKET}
...
first_background_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/evolve/populations/stage=0+what=generated/stint=${STINT}/series=\${cur_series}/a=population+proc=0+series=\${cur_series}+stint=${STINT}+thread=0+variation=master+ext=.bin.xz"
first_background_attrs: "dose=2+filter_except_lowest_root+"
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/bioticbackground-noncritical-phenotypeneutral-nopinterspersion-competitions/stage=7+what=generated/stint=${STINT}/series=\${cur_series}/"
replicate: \${cur_interspersion_step}
endeavor: \${ENDEAVOR}
stint: ${STINT}
series: \${cur_series}
run_seconds: 600
...
\${REPRO_WORK_DIRECTORY}/dishtiny/script/slurm_stoker_containerized_kickoff.sh "${BUCKET}" "${CONTAINER_TAG}" "${REPO_SHA}" "bb-noncritical-phenotypeequivalent-nopinterspersion-competition~configpack%${CONFIGPACK}~series%${SERIES%% *}...~stint%${STINT}"
