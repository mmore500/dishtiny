...
################################################################################
echo
echo "running controlcompetition-extendedkickoff.sh"
echo "-----------------------------------"
################################################################################
...
for STINT in ${STINTS}; do
  for REPLICATE in {0..19}; do
    ...
    FIRST_COMPETITOR="s3://${BUCKET}/endeavor=\${ENDEAVOR}/genomes/stage=0+what=generated/stint=\${STINT}/series=${SERIES}/a=genome+criteria=abundance+morph=wildtype+proc=0+series=${SERIES}+stint=\${STINT}+thread=0+variation=master+ext=.json.gz"
...
output_url: "s3://${BUCKET}/endeavor=\${ENDEAVOR}/control-extended-competitions/stage=1+what=generated/stint=\${STINT}/series=${SERIES}/"
replicate: "\${REPLICATE}"
run_seconds: 3600
endeavor: "\${ENDEAVOR}"
stint: "\${STINT}"
