...
################################################################################
echo
echo "running lowestandhighestroot-multiculture-extendedkickoff.sh"
echo "------------------------------------------------------------"
################################################################################
...
for STINT in ${STINTS}; do
  ...
  j2 --format=yaml -o "\${JOB_SCRIPT}" "dishtiny/slurm/multicultures/lowestandhighestroot-multiculture-extendedjob.slurm.sh.jinja" << J2_HEREDOC_EOF
bucket: "${BUCKET}"
