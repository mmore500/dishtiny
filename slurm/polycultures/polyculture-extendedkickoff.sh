...
################################################################################
echo
echo "running polyculture-extendedkickoff.sh"
echo "------------------------"
################################################################################
...
for STINT in ${STINTS}; do
  ...
  j2 --format=yaml -o "\${JOB_SCRIPT}" "dishtiny/slurm/polycultures/polyculture-extendedjob.slurm.sh.jinja" << J2_HEREDOC_EOF
bucket: "${BUCKET}"
