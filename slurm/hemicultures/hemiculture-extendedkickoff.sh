...
################################################################################
echo
echo "running hemiculture-extendedkickoff.sh"
echo "------------------------"
################################################################################
...
for STINT in ${STINTS}; do
  ...
  j2 --format=yaml -o "\${JOB_SCRIPT}" "dishtiny/slurm/hemicultures/hemiculture-extendedjob.slurm.sh.jinja" << J2_HEREDOC_EOF
bucket: "${BUCKET}"
