...
################################################################################
echo
echo "running highestroot-hemiculturekickoff.sh"
echo "------------------------"
################################################################################
...
for just_one_series in ${SERIES}; do
  ...
  j2 --format=yaml -o "\${JOB_SCRIPT}" "dishtiny/slurm/hemicultures/highestroot-hemiculturejob.slurm.sh.jinja" << J2_HEREDOC_EOF
bucket: "${BUCKET}"
