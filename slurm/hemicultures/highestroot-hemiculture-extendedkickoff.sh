...
################################################################################
echo
echo "running highestroot-hemiculture-extendedkickoff.sh"
echo "------------------------"
################################################################################
...
if (( "$#" < 6 )); then
  echo "USAGE: [bucket] [configpack] [container_tag] [repo_sha] [series] [stint...]"
  exit 1
fi
...
for STINT in ${STINTS}; do
  ...
  j2 --format=yaml -o "\${JOB_SCRIPT}" "dishtiny/slurm/hemicultures/highestroot-hemiculture-extendedjob.slurm.sh.jinja" << J2_HEREDOC_EOF
bucket: "${BUCKET}"
